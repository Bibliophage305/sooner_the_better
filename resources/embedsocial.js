! function(t, e) {
    ...
    m.prototype.init = function() {
        var t = this;
        this.el || this.create();
        var e, o, n, s, d, l, a = (e = function(e) {
...
if (!document.getElementById("EmbedSocialIFrameLightboxCSS")) {
    ...
    cssEmbed.id = "EmbedSocialIFrameLightboxCSS", cssEmbed.rel = "stylesheet", cssEmbed.href = "https://embedsocial.com/cdn/iframe-lightbox.min.css?v=1.6";
...
EMBEDSOCIALHASHTAG = {
    getEmbedData: function(albumRef, albumDiv) {
        ...
        if (iframes.length <= 0) {
            ...
            EMBEDSOCIALHASHTAG.initResizeFrame(albumRef);
    ...
    initResizeFrame: function(albumRef) {
        if (document.getElementById("EmbedSocialIFrame") && "function" === typeof iFrameResize) {
            ...
            var ifr = document.getElementById("embedIFrame_" + albumRef);
        ...
        if (!document.getElementById("embed-lightbox-" + data.albumRef)) {
            ...
            el.lightbox = new EmbedSocialIframeLightbox(el, data.albumRef, {
                onLoaded: function(iframe) {
                    EMBEDSOCIALHASHTAG.initResizeFrame(data.albumRef);

                    function lightBoxIframeResize() {
                        var windowHeight = window.innerHeight || document.documentElement.clientHeight;
                        var windowWidth = window.innerWidth || document.documentElement.clientWidth;
                        var el = document.getElementById("embed-lightbox-iframe-" + data.albumRef);
                        if (windowHeight > 1800) {
                            windowHeight = 1800
                        }
                        if (typeof el !== "undefined" && el !== null) {
                            if (windowWidth < 911) {
                                el.style.height = parseInt(windowHeight) + "px"
                            } else {
                                el.style.height = parseInt(windowHeight * .95) + "px"
                            }
                        }
                    }
    ...
    closeLightBox: function(data) {
        ...
        if (data.activePage && data.activePage != -1) {
            document.getElementById("embedIFrame_" + data.albumRef).src = "https://embedsocial.com/api/pro_hashtag/" + data.albumRef + "/" + data.activePage
        }
    },
    ...
    removeFreeBanner: function(data) {
        if (data.version !== "free") {
            var iframe = document.getElementById("embedIFrame_" + data.albumRef);
            if (iframe) {
    ...
    freeLinkClass: function(albumDiv) {
        ...
        for (i = 0; i < freeLinks.length; i++) {
            freeLinks[i].className = albumDiv.offsetWidth < 768 ? "feed-powered-by-es feed-powered-by-es-center" : "feed-powered-by-es"
        }
    }
};
if ("IntersectionObserver" in window) {
    function callVisible(e, t) {
        for (i in e) e[i].isIntersecting && EMBEDSOCIALHASHTAG.getEmbedData(e[i].target.getAttribute("data-ref"), e[i].target)
    }
}
